  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.user) {
        ...
        await supabase.from("profiles").upsert({ id: data.user.id, first_name, last_name, phone: phone || null });
      }
    ...
    <div className="min-h-screen grid place-items-center bg-background px-4 py-12">
      ...
      <div className="w-full max-w-md rounded-2xl glass p-8">
        ...
        <Button onClick={google} variant="outline" className="w-full mt-6 h-11" type="button">
          <svg viewBox="0 0 24 24" className="size-4 mr-2"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.25 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.1s.13-1.44.35-2.1V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.83z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.83C6.71 7.31 9.14 5.38 12 5.38z"/></svg>
          Continue with Google
        </Button>
        <div className="my-4 flex items-center gap-2 text-xs text-muted-foreground">
          <div className="h-px flex-1 bg-[--pink]/30" /> or <div className="h-px flex-1 bg-[--pink]/30" />
        </div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          Already have one? <Link to="/account/login" search={search as any} className="text-fire font-medium underline">Sign in</Link>
        </p>
        <p className="mt-2 text-center"><Link to="/" className="text-xs text-muted-foreground hover:underline">← Back to site</Link></p>
      </div>
    </div>
  );
}
