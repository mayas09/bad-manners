...
function generatePickupSlots(hoursStr: string): { value: string; label: string }[] {
  ...
  const now = new Date();
  const open = new Date(now); open.setHours(openT.h, openT.min, 0, 0);
  const close = new Date(now); close.setHours(closeT.h, closeT.min, 0, 0);
  let start = new Date(Math.max(now.getTime() + 15 * 60 * 1000, open.getTime()));
...
function CheckoutPage() {
  ...
  useEffect(() => {
    if (!auth.loading && !auth.user) nav({ to: "/account/login", search: { next: "/checkout" } as any });
  ...
  const slots = useMemo(() => {
    ...
    const label = dayIdx === 0 || dayIdx === 6 ? content.hours.find((h) => /sat|sun/i.test(h.label)) : content.hours.find((h) => /mon|fri|week/i.test(h.label));
    return generatePickupSlots(label?.hours_text ?? "8:00 AM – 3:00 PM");
  }, [content.hours, content.loaded]);

  useEffect(() => { if (slots.length && !pickup) setPickup(slots[0].value); }, [slots, pickup]);
  ...
  async function placeOrder(e: React.FormEvent) {
    ...
    try {
      ...
      if (error || !order) { toast.error(error?.message || "Order failed"); return; }
      ...
      const res = await createSession({ data: { orderId: order.id, originUrl: window.location.origin } });
      if (res?.url) {
        cart.clear();
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-[--pink]/20 py-4 px-4">
        <div className="mx-auto max-w-4xl flex items-center justify-between">
          ...
          <Link to="/" className="text-sm text-muted-foreground hover:underline">← Keep shopping</Link>
        </div>
      </header>
      <main className="mx-auto max-w-4xl px-4 py-8 grid gap-8 md:grid-cols-[1fr_360px]">
        <form onSubmit={placeOrder} className="space-y-6">
          ...
          <section className="glass rounded-2xl p-5 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5"><Label>Full name</Label><Input value={name} onChange={(e) => setName(e.target.value)} required maxLength={100} /></div>
              <div className="grid gap-1.5"><Label>Phone</Label><Input value={phone} onChange={(e) => setPhone(e.target.value)} required type="tel" maxLength={20} /></div>
            </div>
          </section>
          <section className="glass rounded-2xl p-5 space-y-3">
            ...
            {slots.length === 0 ? (
              ...
              <select value={pickup} onChange={(e) => setPickup(e.target.value)} className="w-full rounded-md border border-slate-200 bg-white px-3 py-2">
          ...
          <section className="glass rounded-2xl p-5 space-y-2">
            ...
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={500} placeholder="Anything the barista should know?" />
          </section>
          <Button type="submit" disabled={busy || cart.items.length === 0 || slots.length === 0} className="w-full h-12 bg-fire text-white text-base">
        ...
        <aside className="glass rounded-2xl p-5 h-fit sticky top-4">
          ...
          <div className="mt-4 pt-3 border-t border-[--pink]/20 flex justify-between font-display text-xl">
            <span>Total</span><span className="text-fire">{formatCents(cart.subtotalCents)}</span>
          </div>
        </aside>
      </main>
    </div>
