-- Atomically finalize a paid order (upsert order + replace its order_items)
-- so that near-simultaneous duplicate finalizeOrder calls (double-tab, client
-- retry) cannot race and insert duplicate order_items. An advisory lock keyed
-- on the order id serializes concurrent calls for the same order, and the
-- payment_status is re-checked once the lock is held so only the first caller
-- does any writing.
